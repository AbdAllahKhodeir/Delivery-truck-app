...
for index in range(len(distances.first_truck_index())):
    try:
        ...
        csv_reader.get_hash_map().update(int(distances.first_truck_list()[index][0]), first_delivery)
    except IndexError:
        pass
...
for index in range(len(distances.second_truck_index())):
    try:
        total_distance_2 = distances.get_distance(int(distances.second_truck_index()[index]),
                                                 int(distances.second_truck_index()[index + 1]), total_distance_2)

        deliver_package = distances.get_time(distances.get_current_distance(int(distances.second_truck_index()[index]),
                                                                          int(distances.second_truck_index()[
                                                                                  index + 1])), second_leave_times)
        distances.second_truck_list()[index][10] = (str(deliver_package))
        csv_reader.get_hash_map().update(int(distances.second_truck_list()[index][0]), second_delivery)
    except IndexError:
        pass
...
for index in range(len(distances.third_truck_index())):
    try:
        total_distance_3 = distances.get_distance(int(distances.third_truck_index()[index]),
                                                 int(distances.third_truck_index()[index + 1]), total_distance_3)

        deliver_package = distances.get_time(distances.get_current_distance(int(distances.third_truck_index()[index]),
                                                                          int(distances.third_truck_index()[index + 1])),
                                            third_leave_times)
        distances.third_truck_list()[index][10] = (str(deliver_package))
        csv_reader.get_hash_map().update(int(distances.third_truck_list()[index][0]), third_delivery)
    except IndexError:
        pass
